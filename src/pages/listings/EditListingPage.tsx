...
export function EditListingPage() {
  ...
  const loadProperty = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        sqft: data.size_sqft?.toString() || '',
        parking_spaces: data.parking_spaces?.toString() || '',
        loading_docks: data.loading_docks?.toString() || '',
        floor_number: data.floor_number?.toString() || '',
        total_floors: data.total_floors?.toString() || '',
        zoning: data.zoning || '',
        monthly_maintenance: data.monthly_maintenance?.toString() || '',
        property_type: data.property_type,
        property_category: data.property_category as PropertyCategory,
        property_general_type: data.property_general_type as PropertyGeneralType,
        property_style: data.property_style as PropertyStyle,
      ...
      if (featuresData) {
        setSelectedFeatures(new Set(featuresData.map(f => f.feature_name)));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let displayMessage = 'Failed to update listing. Please try again.';
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Card className="p-6 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">The property you're trying to edit doesn't exist or you don't have permission to edit it.</p>
          </Card>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      ...
      <SuccessModal
        ...
        message="Your property listing has been updated and the changes are now live."
      />
    </div>
  );
}
